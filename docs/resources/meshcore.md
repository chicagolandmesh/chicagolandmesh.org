---
title: MeshCore Resources
hide:
  - navigation
  - toc
tags:
  - Info
  - MeshCore
---

# MeshCore Resources

<div class="grid cards small" markdown>

-   [:material-access-point-network:{ .lg .top } __MeshCore Home__](https://meshcore.io/)

    Official MeshCore project site and primary entry point.

    ![Link Preview](https://image.thum.io/get/width/700/maxAge/672/allowJPG/noanimate/http://meshcore.io/)

-   [:fontawesome-brands-github:{ .lg .top } __MeshCore GitHub__](https://github.com/meshcore-dev/MeshCore)

    Browse source code, releases, and project activity for MeshCore.

    ![Link Preview](https://image.thum.io/get/width/700/maxAge/672/allowJPG/noanimate/http://github.com/meshcore-dev/MeshCore)

-   [:fontawesome-brands-github:{ .lg .top } __Awesome MeshCore__](https://github.com/samuk/awesome-meshcore)

    GitHub Repo with very useful and up to date MeshCore tools.

    ![Link Preview](https://image.thum.io/get/width/700/maxAge/672/allowJPG/noanimate/http://github.com/samuk/awesome-meshcore)    

</div>


## Tools and Apps

<div class="grid cards small" markdown>

-   [Official MeshCore Flasher](https://flasher.meshcore.io)
    ![Link Preview](https://image.thum.io/get/width/700/maxAge/672/allowJPG/noanimate/http://flasher.meshcore.io)
    
-   [MeshCore MQTT Observer Flasher](https://observer.gessaman.com)
    ![Link Preview](https://image.thum.io/get/width/700/maxAge/672/allowJPG/noanimate/http://observer.gessaman.com)    

-   [MeshCore Analyzer](https://analyzer.letsmesh.net)
    ![Link preivew](https://image.thum.io/get/width/700/maxAge/672/allowJPG/noanimate/http://analyzer.letsmesh.net)

-   [Official MeshCore Map](https://map.meshcore.io)
    ![Link preivew](https://image.thum.io/get/width/700/maxAge/672/allowJPG/noanimate/http://map.meshcore.io)

-   [CHGO Mesh Planner](https://plan.chgomesh.net/)
    ![Link preview](https://image.thum.io/get/width/700/maxAge/672/allowJPG/noanimate/http://plan.chgomesh.net/)

-   [Live ChiMeshCore Map](https://live.chimesh.org/)
    ![Link Preview](https://image.thum.io/get/width/700/maxAge/672/allowJPG/noanimate/http://live.chimesh.org/)    

-   [Mesh Community Planner](https://github.com/PapaSierra555/MeshCommunityPlanner)
    ![Link preview](https://image.thum.io/get/width/700/maxAge/672/allowJPG/noanimate/http://github.com/PapaSierra555/MeshCommunityPlanner)

-   [MeshCore Android App (Google Play)](https://play.google.com/store/apps/details?id=com.liamcottle.meshcore.android)
    ![Link Preview](https://image.thum.io/get/width/700/maxAge/672/allowJPG/noanimate/http://play.google.com/store/apps/details?id=com.liamcottle.meshcore.android)

-   [MeshCore Android APK (Direct)](https://files.liamcottle.net/MeshCore/)
    ![Link Preview](https://image.thum.io/get/width/700/maxAge/672/allowJPG/noanimate/http://files.liamcottle.net/MeshCore/)

-   [MeshCore CLI](https://github.com/meshcore-dev/meshcore-cli)
    ![Link preivew](https://image.thum.io/get/width/700/maxAge/672/allowJPG/noanimate/http://github.com/meshcore-dev/meshcore-cli)

-   [MeshCore Configuration Tool](https://config.meshcore.io)
    ![Link Preview](https://image.thum.io/get/width/700/maxAge/672/allowJPG/noanimate/http://config.meshcore.io)

-   [MeshCore Documentation](https://docs.meshcore.io/)
    ![Link Preview](https://image.thum.io/get/width/700/maxAge/672/allowJPG/noanimate/http://docs.meshcore.io/)

-   [MeshCore for Home Assistant](https://github.com/meshcore-dev/meshcore-ha)
    ![Link preview](https://image.thum.io/get/width/700/maxAge/672/allowJPG/noanimate/http://github.com/meshcore-dev/meshcore-ha)

-   [MeshCore iOS App](https://apps.apple.com/us/app/meshcore/id6742354151)
    ![Link Preview](https://image.thum.io/get/width/700/maxAge/672/allowJPG/noanimate/http://apps.apple.com/us/app/meshcore/id6742354151)

-   [MeshCore Web Client (Community)](https://meshcore.liamcottle.net/)
    ![Link Preview](https://image.thum.io/get/width/700/maxAge/672/allowJPG/noanimate/http://meshcore.liamcottle.net/)

-   [MeshCore Web Client (Official)](https://app.meshcore.nz/)
    ![Link Preview](https://image.thum.io/get/width/700/maxAge/672/allowJPG/noanimate/http://app.meshcore.nz/)

-   [RF Index — MeshCore Devices](https://www.rfindex.com/mesh/devices?firmware=MeshCore)
    ![Link Preview](https://image.thum.io/get/width/700/maxAge/672/allowJPG/noanimate/http://www.rfindex.com/mesh/devices?firmware=MeshCore)

</div>


## MQTT Related

<div class="grid cards small" markdown>

-   [Mesh Mapper](https://meshmapper.net)
    ![Link preivew](https://image.thum.io/get/width/700/maxAge/672/allowJPG/noanimate/http://meshmapper.net)

-   [Mesh Mapper Chicagoland Region](https://chgo.meshmapper.net)
    ![Link preivew](https://image.thum.io/get/width/700/maxAge/672/allowJPG/noanimate/wait/10/http://chgo.meshmapper.net)

-   [Mesh Mapper Wiki](https://wiki.meshmapper.net)
    ![Link preivew](https://image.thum.io/get/width/700/maxAge/672/allowJPG/noanimate/http://wiki.meshmapper.net)

-   [MeshCore MQTT Broker GitHub](https://github.com/michaelhart/meshcore-mqtt-broker)
    ![Link preivew](https://image.thum.io/get/width/700/maxAge/672/allowJPG/noanimate/http://github.com/michaelhart/meshcore-mqtt-broker)

-   [MeshCore Live Map GitHub](https://github.com/yellowcooln/meshcore-mqtt-live-map)
    ![Link preivew](https://image.thum.io/get/width/700/maxAge/672/allowJPG/noanimate/http://github.com/yellowcooln/meshcore-mqtt-live-map)

-   [MeshCore Health Check GitHub](https://github.com/yellowcooln/meshcore-mqtt-live-map)
    ![Link preivew](https://image.thum.io/get/width/700/maxAge/672/allowJPG/noanimate/http://github.com/yellowcooln/meshcore-mqtt-live-map)

-   [CoreScope GitHub](https://github.com/Kpa-clawbot/CoreScope)
    ![Link preivew](https://image.thum.io/get/width/700/maxAge/672/allowJPG/noanimate/http://github.com/Kpa-clawbot/CoreScope)

-   [meshcoretomqtt (Computer Bridge)](https://github.com/Cisien/meshcoretomqtt)
    ![Link Preview](https://image.thum.io/get/width/700/maxAge/672/allowJPG/noanimate/http://github.com/Cisien/meshcoretomqtt)

-   [meshcore-packet-capture (Companion Bridge)](https://github.com/agessaman/meshcore-packet-capture)
    ![Link Preview](https://image.thum.io/get/width/700/maxAge/672/allowJPG/noanimate/http://github.com/agessaman/meshcore-packet-capture)


</div>


## Firmware and Hardware

<div class="grid cards small" markdown>

-   [EasySkyMesh (Power Saving Firmware)](https://github.com/IoTThinks/EasySkyMesh)
    ![Link Preview](https://image.thum.io/get/width/700/maxAge/672/allowJPG/noanimate/http://github.com/IoTThinks/EasySkyMesh)

-   [nRF Connect — Android](https://play.google.com/store/apps/details?id=no.nordicsemi.android.mcp)
    ![Link Preview](https://image.thum.io/get/width/700/maxAge/672/allowJPG/noanimate/http://play.google.com/store/apps/details?id=no.nordicsemi.android.mcp)

-   [nRF Connect — iOS](https://apps.apple.com/gb/app/nrf-connect-for-mobile/id1054362403)
    ![Link Preview](https://image.thum.io/get/width/700/maxAge/672/allowJPG/noanimate/http://apps.apple.com/gb/app/nrf-connect-for-mobile/id1054362403)

-   [nRF Device Firmware Update — Android](https://play.google.com/store/apps/details?id=no.nordicsemi.android.dfu)
    ![Link Preview](https://image.thum.io/get/width/700/maxAge/672/allowJPG/noanimate/http://play.google.com/store/apps/details?id=no.nordicsemi.android.dfu)

-   [nRF Device Firmware Update — iOS](https://apps.apple.com/sa/app/device-firmware-update/id1624454660)
    ![Link Preview](https://image.thum.io/get/width/700/maxAge/672/allowJPG/noanimate/http://apps.apple.com/sa/app/device-firmware-update/id1624454660)

-   [openHop Modem Flasher](https://flasher.openhop.dev/)
    ![Link Preview](https://image.thum.io/get/width/700/maxAge/672/allowJPG/noanimate/http://flasher.openhop.dev/)

-   [openHop Repeater Documentation](https://docs.openhop.dev/projects/openhop-repeater/what-is-openhop-repeater/)
    ![Link Preview](https://image.thum.io/get/width/700/maxAge/672/allowJPG/noanimate/http://docs.openhop.dev/projects/openhop-repeater/what-is-openhop-repeater/)

-   [openHop Repeater GitHub](https://github.com/openhop-dev/openhop_repeater)
    ![Link Preview](https://image.thum.io/get/width/700/maxAge/672/allowJPG/noanimate/http://github.com/openhop-dev/openhop_repeater)

-   [OTAFIX Bootloader (nRF52)](https://github.com/oltaco/Adafruit_nRF52_Bootloader_OTAFIX)
    ![Link Preview](https://image.thum.io/get/width/700/maxAge/672/allowJPG/noanimate/http://github.com/oltaco/Adafruit_nRF52_Bootloader_OTAFIX)

</div>


## Other

<div class="grid cards small" markdown>

-   [MeshOS KeyGen](https://meshoskey.com/)
    ![Link preivew](https://image.thum.io/get/width/700/maxAge/672/allowJPG/noanimate/http://meshoskey.com/)

-   [MeshCore Web KeyGen](https://github.com/agessaman/meshcore-web-keygen)
    ![Link preivew](https://image.thum.io/get/width/700/maxAge/672/allowJPG/noanimate/http://github.com/agessaman/meshcore-web-keygen)

</div>
